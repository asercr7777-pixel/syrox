...
import { ShadowCoach } from './components/ShadowCoach';
...
const StoryMode = lazy(() => import('./views/StoryModeSimple'));
...
function AppContent() {
 ...
 return <div className="min-h-screen isolate" data-theme={state.theme}><Background/><Navigation current={view} onNavigate={handleNavigate}/><ToastContainer/><Confetti/><StoryProgressBridge/><main className="relative z-10 lg:ml-64 pt-16 lg:pt-6 px-3 sm:px-4 pb-24 lg:pb-8 max-w-6xl mx-auto overflow-x-hidden">{view==='settings'&&<SettingsAppearance/>}<Suspense fallback={<PageLoader/>}>{view==='dashboard'&&<Dashboard onNavigate={handleNavigate}/>} {view==='dashboard'&&!isInstalled&&<div className="flex justify-center mt-5 mb-1"><InstallButton isInstallable={isInstallable} isInstalled={isInstalled} onInstall={promptInstall}>Install SYROX</InstallButton></div>} {view==='tasks'&&<Tasks/>} {view==='story'&&<StoryMode/>} {view==='skilltree'&&<SkillTree/>} {view==='workout'&&<WorkoutWithAIPlan/>} {view==='shadowai'&&<ShadowCoach><ShadowAI/></ShadowCoach>} {view==='dungeons'&&<Dungeons/>} {view==='profile'&&<Profile/>} {view==='marketplace'&&<Marketplace/>} {view==='inventory'&&<Inventory/>} {view==='achievements'&&<Achievements/>} {view==='leaderboard'&&<Leaderboard/>} {view==='community'&&<Community/>} {view==='settings'&&<Settings/>} {view==='iteminspection'&&<ItemInspection itemId="" category="weapon" onBack={()=>handleNavigate('inventory')}/>}</Suspense></main></div>;
}
