...
export function Navigation({ current, onNavigate }: NavigationProps) {
  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-white/10 bg-black/70 p-4 backdrop-blur-xl lg:block">
      <div className="mb-6 flex items-center gap-3 px-2"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-ember-500/15 text-xl">⚒️</div><div><div className="font-black tracking-[0.2em] text-white">FORGED</div><div className="text-[10px] uppercase tracking-widest text-ink-500">Discipline System</div></div></div>
    ...
    <div className="fixed inset-x-0 top-0 z-50 flex h-16 items-center justify-between border-b border-white/10 bg-black/75 px-4 backdrop-blur-xl lg:hidden"><div className="font-black tracking-[0.2em] text-white">FORGED</div><button onClick={() => setMobileOpen((v) => !v)} className="rounded-xl p-2 text-ink-300 hover:bg-white/10">{mobileOpen ? <X size={22} /> : <Menu size={22} />}</button></div>
