import { LayoutDashboard, CheckSquare, BookOpen, Dumbbell, User, Store, Backpack, Trophy, Users, Settings, Menu, X, LogOut, Sparkles, Swords, Map, Network, Palette } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard }, { id: 'tasks', label: 'Tasks', icon: CheckSquare }, { id: 'story', label: 'Story Mode', icon: BookOpen },
  { id: 'workout', label: 'Workout', icon: Dumbbell }, { id: 'dungeons', label: 'Dungeons', icon: Swords }, { id: 'worldmap', label: 'World Map', icon: Map }, { id: 'skilltree', label: 'Skill Tree', icon: Network },
  { id: 'profile', label: 'Hunter Profile', icon: User }, { id: 'marketplace', label: 'Marketplace', icon: Store }, { id: 'inventory', label: 'Inventory', icon: Backpack }, { id: 'achievements', label: 'Achievements', icon: Trophy },
  { id: 'leaderboard', label: 'Leaderboard', icon: Users }, { id: 'shadow', label: 'Shadow AI', icon: Sparkles }, { id: 'settings', label: 'Settings', icon: Settings },
];

const THEMES: { id: SiteTheme; name: string; colors: string }[] = [
  { id: 'shadow', name: 'Shadow', colors: 'linear-gradient(135deg,#8b5cf6,#111827)' },
  { id: 'ember', name: 'Ember', colors: 'linear-gradient(135deg,#ff7a18,#431407)' },
  { id: 'frost', name: 'Frost', colors: 'linear-gradient(135deg,#38bdf8,#082f49)' },
  { id: 'ocean', name: 'Ocean', colors: 'linear-gradient(135deg,#2563eb,#082f49)' },
  { id: 'emerald', name: 'Emerald', colors: 'linear-gradient(135deg,#10b981,#052e16)' },
  { id: 'crimson', name: 'Crimson', colors: 'linear-gradient(135deg,#e11d48,#4c0519)' },
  { id: 'royal', name: 'Royal', colors: 'linear-gradient(135deg,#6366f1,#581c87)' },
  { id: 'gold', name: 'Gold', colors: 'linear-gradient(135deg,#f59e0b,#451a03)' },
];
...
export function Navigation({ current, onNavigate }: NavigationProps) {
  ...
  const navButton = (item: NavItem, mobile = false) => { const Icon = item.icon; const active = current === item.id || (item.id === 'worldmap' && current === 'story'); return <button key={item.id} onClick={() => handleNav(item.id)} className={`group w-full flex items-center gap-3 rounded-xl text-sm font-medium transition-all duration-200 ${mobile ? 'mb-1.5 px-3 py-3' : 'px-3 py-2.5'} ${active ? 'bg-gradient-to-r from-ember-500/15 via-ember-500/8 to-transparent text-ember-300 border border-ember-500/25 shadow-[inset_3px_0_0_rgba(255,122,24,0.9),0_6px_20px_rgba(0,0,0,0.16)]' : 'text-ink-300 border border-transparent hover:bg-white/[0.045] hover:text-white hover:border-white/[0.06]'}`}><Icon size={18} strokeWidth={active ? 2.3 : 1.8} className={`shrink-0 transition-transform duration-200 ${active ? 'text-ember-400' : 'text-ink-500 group-hover:text-ink-200 group-hover:scale-105'}`} /><span>{item.label}</span></button>; };
  return <>
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-white/[0.08] bg-black/90 p-4 lg:block">
      <div className="mb-6 px-2 pt-1">{brand}</div>
      <div className="mb-4 rounded-2xl border border-white/[0.08] bg-black/60 p-3.5">
        <div className="flex items-center gap-3">
          <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-ember-500/20 bg-ember-500/10 text-xl overflow-hidden">{/^https?:\/\//.test(state.avatar) ? <img src={state.avatar} alt="Profile" className="h-full w-full object-cover" /> : state.avatar}</div>
          ...
          <button onClick={() => setThemeOpen((v) => !v)} className="rounded-lg p-1.5 text-ink-500 hover:bg-white/5 hover:text-ink-100" title="Change theme"><Palette size={15} /></button>
        </div>
      ...
      <nav className="space-y-1 overflow-y-auto pr-1" style={{ maxHeight: 'calc(100vh - 255px)' }}>{NAV_ITEMS.map((item) => navButton(item))}</nav>
    ...
    {mobileOpen && <div className="fixed inset-0 z-40 bg-black/90 pt-16 lg:hidden"><nav className="max-h-[calc(100vh-4rem)] overflow-y-auto border-b border-white/[0.08] bg-black/95 p-4 shadow-2xl"><div className="mb-3 rounded-xl border border-white/10 bg-black/60 p-3"><div className="flex items-center justify-between"><span className="text-xs text-ink-400">Theme</span><div className="flex gap-1">{THEMES.map((t) => <button key={t.id} onClick={() => updateProfile({ theme: t.id })} title={t.name} className={`h-6 w-6 rounded-md border ${state.theme === t.id ? 'border-white' : 'border-white/10'}`} style={{ background: t.colors }} />)}</div></div></div>{NAV_ITEMS.map((item) => navButton(item, true))}<button onClick={handleSignOut} className="mt-2 flex w-full items-center gap-3 rounded-xl border border-transparent px-3 py-3 text-sm text-ink-500 transition hover:border-white/[0.06] hover:bg-white/[0.04] hover:text-white"><LogOut size={18} /> Sign out</button></nav></div>}
  </>;
}
