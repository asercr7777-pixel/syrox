import { LayoutDashboard, CheckSquare, BookOpen, Dumbbell, User, Store, Backpack, Trophy, Users, Settings, Menu, X, LogOut, Palette, MessageCircle, Swords } from 'lucide-react';
...
export function Navigation({ current, onNavigate }: NavigationProps) {
  ...
  const brand = <div className="select-none font-black text-white tracking-[0.30em] leading-none text-[1.15rem] sm:text-[1.25rem]">FORGED</div>;
  const navButton = (item: NavItem, mobile = false) => { const Icon = item.icon; const active = current === item.id; return <button key={item.id} onClick={() => handleNav(item.id)} className={`group w-full flex shrink-0 items-center gap-3 rounded-xl text-sm font-medium transition-all duration-200 ${mobile ? 'mb-1 px-3 py-2.5' : 'px-3 py-2.5'} ${active ? 'bg-gradient-to-r from-ember-500/15 via-ember-500/8 to-transparent text-ember-300 border border-ember-500/25 shadow-[inset_3px_0_0_rgba(255,122,18,0.9),0_6px_20px_rgba(0,0,0,0.16)]' : 'text-ink-300 border border-transparent hover:bg-white/[0.045] hover:text-white hover:border-white/[0.06]'}`}><Icon size={18} strokeWidth={active ? 2.3 : 1.8} className={`shrink-0 transition-transform duration-200 ${active ? 'text-ember-400' : 'text-ink-500 group-hover:text-ink-200 group-hover:scale-105'}`} /><span className="truncate">{item.label}</span></button>; };
  return <>
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-white/[0.08] bg-black/90 p-3.5 lg:flex lg:flex-col">
      <div className="shrink-0 px-2 pt-1 pb-4">{brand}</div>
      <div className="shrink-0 rounded-2xl border border-white/[0.08] bg-black/60 p-3">
        <div className="flex items-center gap-2.5"><UserAvatar avatar={state.avatar} rank={rank} size="sm" /><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold text-white">{state.username}</div><div className="mt-1 flex items-center gap-2 text-xs text-ink-400"><RankBadge rank={rank} compact /> <span>{state.streak}🔥</span></div></div><button onClick={() => setThemeOpen((v) => !v)} className="shrink-0 rounded-lg p-1.5 text-ink-500 hover:bg-white/5 hover:text-ink-100" title="Change theme"><Palette size={15} /></button></div>
        <XpBar xp={state.xp} className="mt-3" />{title && <div className="mt-2 truncate text-xs font-semibold" style={{ color: titleMeta?.color }}>{title.name}</div>}{aura && <div className="mt-1 truncate text-[10px] text-ink-500">{aura.name}</div>}
        {themeOpen && <div className="mt-2.5 grid grid-cols-4 gap-1.5 rounded-xl border border-white/10 bg-black/80 p-2">{THEMES.map((t) => <button key={t.id} onClick={() => { updateProfile({ theme: t.id }); playSound('click'); setThemeOpen(false); }} title={t.name} className={`h-7 rounded-lg border ${state.theme === t.id ? 'border-white' : 'border-white/10'}`} style={{ background: t.colors }} />)}</div>}
      </div>
      <nav className="mt-3 min-h-0 flex-1 space-y-0.5 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">{NAV_ITEMS.map((item) => navButton(item))}</nav>
      <div className="shrink-0 border-t border-white/[0.08] pt-2.5 mt-2.5"><button onClick={handleSignOut} className="flex w-full items-center gap-3 rounded-xl border border-transparent px-3 py-2.5 text-sm text-ink-500 transition hover:border-white/[0.06] hover:bg-white/[0.04] hover:text-white"><LogOut size={18} /> Sign out</button></div>
    </aside>
    ...
    {mobileOpen && <div className="fixed inset-0 z-40 bg-black/90 pt-16 lg:hidden"><nav className="flex h-[calc(100vh-4rem)] flex-col overflow-hidden border-b border-white/[0.08] bg-black/95 p-3 shadow-2xl"><div className="mb-2.5 shrink-0 rounded-xl border border-white/10 bg-black/60 p-2.5"><div className="flex items-center justify-between"><span className="text-xs text-ink-400">Theme</span><div className="flex gap-1">{THEMES.map((t) => <button key={t.id} onClick={() => { updateProfile({ theme: t.id }); setMobileOpen(false); }} title={t.name} className={`h-6 w-6 rounded-md border ${state.theme === t.id ? 'border-white' : 'border-white/10'}`} style={{ background: t.colors }} />)}</div></div></div><div className="min-h-0 flex-1 overflow-y-auto">{NAV_ITEMS.map((item) => navButton(item, true))}</div><div className="mt-2 shrink-0 border-t border-white/[0.08] pt-2"><button onClick={handleSignOut} className="flex w-full items-center gap-3 rounded-xl border border-transparent px-3 py-3 text-sm text-ink-500 transition hover:border-white/[0.06] hover:bg-white/[0.04] hover:text-white"><LogOut size={18} /> Sign out</button></div></nav></div>}
  </>;
}
