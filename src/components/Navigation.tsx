  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-white/[0.08] bg-black/90 p-3.5 lg:flex lg:flex-col">
      ...
      <div className="shrink-0 rounded-2xl border border-white/[0.08] bg-black/60 p-3">
        ...
        <XpBar xp={state.xp} />{title && <div className="mt-2 truncate text-xs font-semibold" style={{ color: titleMeta?.color }}>{title.name}</div>}{aura && <div className="mt-1 truncate text-[10px] text-ink-500">{aura.name}</div>}
