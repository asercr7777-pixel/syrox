...
export function SixDayWorkout() {
  ...
  const syncStoryAfterWorkout = (durationSeconds: number) => {
    ...
    chapter.missions.filter((mission) => mission.type === 'workout' && !state.storyCompletedMissions[mission.id] && workoutCountToday >= mission.target)
      .forEach((mission) => completeStoryMission(mission.id, { xp: mission.xpReward, coins: 0 }));
  ...
  const saveExercise = () => {
    ...
    const reps = exerciseDraft.reps.trim();
    if (!name || !reps) return;
    const sets = Number.isFinite(exerciseDraft.sets) && exerciseDraft.sets > 0 ? exerciseDraft.sets : 1;
    updateCurrent(d => ({
      ...d,
  ...
  const exerciseModal = exerciseDraft && typeof document !== 'undefined' ? createPortal(
    <div className="fixed inset-0 z-[99999] flex items-start sm:items-center justify-center overflow-y-auto overscroll-contain bg-black/70 p-3 sm:p-5" role="dialog" aria-modal="true" onClick={() => setExerciseDraft(null)}>
      <div className="card !w-[min(92vw,560px)] !max-w-[560px] max-h-[calc(100dvh-1.5rem)] sm:max-h-[88dvh] overflow-y-auto overscroll-contain p-4 sm:p-5 space-y-3 sm:space-y-4 my-0 sm:my-auto" onClick={e => e.stopPropagation()}>
        ...
        <input autoFocus className="input" placeholder="Exercise name" value={exerciseDraft.name} onChange={e => setExerciseDraft({ ...exerciseDraft, name: e.target.value })}/>
  ...
  return <div className="space-y-5">
    ...
    <div className="card-premium p-5 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>{editingDay ? <div className="flex gap-2"><input autoFocus value={dayName} onChange={e => setDayName(e.target.value)} onKeyDown={e => e.key === 'Enter' && saveDayName()} className="input" placeholder="Day name" /><button type="button" onClick={saveDayName} className="btn-primary">Save</button><button type="button" onClick={() => setEditingDay(false)} className="btn-ghost"><X size={16}/></button></div> : <div><div className="flex items-center gap-2"><h2 className="font-display text-2xl font-bold">{current.name}</h2><button type="button" className="btn-ghost p-2" onClick={() => { setDayName(current.name); setEditingDay(true); }} disabled={running}><Pencil size={15}/></button></div><p className="text-sm text-ink-400">Day {active + 1} • {progress}% complete</p></div>}</div>
        <button type="button" onClick={() => setExerciseDraft({ name: '', sets: 3, reps: '8-12', section: 'main' })} className="btn-primary" disabled={running}><Plus size={17}/> Add Exercise</button>
      </div>
