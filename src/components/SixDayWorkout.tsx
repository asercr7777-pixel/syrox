...
export function SixDayWorkout() {
  ...
  useEffect(() => () => { /* timer state intentionally remains local until Finish */ }, []);
  ...
  const finishTimer = () => {
    ...
    const history = [entry, ...loadHistory()].slice(0, 100);
  ...
  return <div className="space-y-5">
    ...
    {(['stretching','main','plyometric'] as Section[]).map(section => { const items = current.exercises.filter(e => e.section === section); return <div key={section} className="card p-4 sm:p-5"><div className="flex items-center justify-between mb-3"><h3 className="font-display font-bold uppercase tracking-wider theme-accent">{labels[section]}</h3><span className="text-xs text-ink-400">{items.length} exercises</span></div>{items.length === 0 ? <p className="text-sm text-ink-400 py-3">No exercises yet.</p> : <div className="space-y-2">{items.map(ex => <div key={ex.id} className={`flex items-center gap-3 rounded-xl border p-3 ${ex.done ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-white/5 bg-ink-950/30'}`}><button onClick={() => toggle(ex.id)} disabled={running} className={`w-6 h-6 rounded-md border-2 flex items-center justify-center ${ex.done ? 'bg-emerald-500 border-emerald-500' : 'border-ink-500'}`}>{ex.done && <Check size={14}/>}</button><div className="flex-1 min-w-0"><p className={`font-medium ${ex.done ? 'line-through text-ink-400' : ''}`}>{ex.name}</p><p className="text-xs text-ink-400">{ex.sets} sets × {ex.reps}</p></div><button className="btn-ghost p-2" onClick={() => setExerciseDraft({ id: ex.id, name: ex.name, sets: ex.sets, reps: ex.reps, section: ex.section })} disabled={running}><Pencil size={14}/></button><button className="btn-ghost p-2 text-danger-400" onClick={() => remove(ex.id)} disabled={running}><Trash2 size={14}/></button></div>)}</div>}</div>; })}
    <button onClick={reset} className="btn-ghost" disabled={running}><RotateCcw size={15}/> Reset six-day setup</button>
