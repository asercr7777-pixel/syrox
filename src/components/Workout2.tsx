...
export function Workout2() {
  ...
  const save = async () => {
    ...
    try {
      ...
      const row = { user_id:(await supabase.auth.getUser()).data.user?.id, exercise_id:exerciseId, day_id:dayId, exercise_type:draft.type, duration_seconds:duration, rest_seconds:rest, notes:draft.notes.trim() || null, image_path:imagePath, updated_at:new Date().toISOString() };
