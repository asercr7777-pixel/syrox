  ...
  return <div className="relative min-h-[480px] overflow-hidden rounded-3xl border border-white/10" style={{ background: bgGradient }}>
    ...
    <div className="relative grid min-h-[480px] items-center gap-6 p-5 md:grid-cols-[210px_1fr] md:p-8">
      ...
      <div className="min-w-0">
        ...
        <AnimatePresence mode="wait"><motion.div key={index} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}><div className="mb-2"><span className={`inline-flex rounded-lg px-3 py-1 text-sm font-bold ${isShadow ? 'border border-violet-400/30 bg-violet-500/10 text-violet-200' : 'border border-ember-500/25 bg-ember-500/10 text-ember-300'}`}>{currentLine.speaker}</span></div><button onClick={advance} className="w-full rounded-2xl border border-white/10 bg-black/70 p-5 text-left shadow-xl backdrop-blur-sm"><p className="text-sm leading-7 text-ink-100 md:text-base">{displayedText}{isTyping && <span className="ml-1 inline-block h-4 w-1.5 animate-pulse bg-ember-400" />}</p>{!isTyping && <div className="mt-3 text-[10px] uppercase tracking-wider text-ink-500">Click to continue ▸</div>}</button></motion.div></AnimatePresence>
        ...
        <div className="mt-4 flex gap-1.5">{lines.map((_, i) => <div key={i} className={`h-1.5 rounded-full ${i === index ? 'w-7 bg-ember-500' : i < index ? 'w-1.5 bg-ember-500/40' : 'w-1.5 bg-white/10'}`} />)}</div>
      </div>
    </div>
  </div>;
}