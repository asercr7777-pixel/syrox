...
export function DisciplineLineChart({ data, height = 260, showToday = true, animate = true }: DisciplineLineChartProps) {
  ...
  const points = useMemo(() => {
    ...
    const barW = data.length > 1 ? Math.max(8, Math.min(22, (innerW - gap * (data.length - 1)) / data.length)) : 24;
    const totalW = data.length * barW + Math.max(0, data.length - 1) * gap;
    const startX = pad.left + Math.max(0, (innerW - totalW) / 2);
    return data.map((d, i) => ({
      ...d,
  ...
  return <div className="w-full">
    ...
    <div className="mb-2 flex flex-col min-[420px]:flex-row min-[420px]:items-center justify-between gap-1.5 text-[10px] sm:text-[11px] text-ink-500">
      ...
      <span>Last 30 days · tap a bar to inspect</span>
    </div>

    <div className="relative overflow-hidden rounded-xl border border-white/[0.06] bg-ink-950/40">
      {points.length === 0 ? <div className="flex items-center justify-center py-16 text-sm text-ink-500">No discipline history yet.</div> : <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={height} preserveAspectRatio="none" className="block w-full select-none" role="img" aria-label={`30 day discipline chart. Today ${metrics.todayScore} percent, 7 day average ${metrics.recentAvg} percent, best ${metrics.best} percent.`}>
        <defs>
          <linearGradient id={`${chartId}-bar`} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#ffb45f"/><stop offset=".55" stopColor="#ff7a18"/><stop offset="1" stopColor="#b9440d"/></linearGradient>
          <linearGradient id={`${chartId}-today`} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#ffe08a"/><stop offset=".55" stopColor="#fbbf24"/><stop offset="1" stopColor="#d97706"/></linearGradient>
          <filter id={`${chartId}-glow`} x="-60%" y="-20%" width="220%" height="150%"><feGaussianBlur stdDeviation="3" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        </defs>

        {[0,25,50,75,100].map(t => { const y = pad.top + innerH - (t / 100) * innerH; return <g key={t}><line x1={pad.left} x2={W-pad.right} y1={y} y2={y} stroke="rgba(255,255,255,.07)" strokeWidth="1" strokeDasharray={t ? '4 8' : undefined}/><text x={pad.left-9} y={y+4} textAnchor="end" fontSize="11" fill="rgba(230,234,245,.4)">{t}</text></g>; })}

        {points.map((p, i) => {
          const today = showToday && p.isToday;
          const active = activeIndex === i;
          const barH = Math.max(3, pad.top + innerH - p.y);
          const barX = p.x - p.barW / 2;
          const radius = Math.min(6, p.barW / 2);
          return <g key={p.date} className="cursor-pointer" onClick={() => setSelected(i === selected ? null : i)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelected(i === selected ? null : i); } }} tabIndex={0} role="button" aria-label={`${formatDate(p.date)}: ${p.score}%`}>
            <rect x={Math.max(pad.left, p.x - 18)} y={pad.top} width="36" height={innerH} fill="transparent"/>
            <rect x={barX} y={p.y} width={p.barW} height={barH} rx={radius} fill={today ? `url(#${chartId}-today)` : `url(#${chartId}-bar)`} opacity={active ? 1 : .78} filter={active || today ? `url(#${chartId}-glow)` : undefined} className={animate ? 'chart-bar-rise' : ''}/>
            {active && <rect x={barX - 1} y={p.y - 2} width={p.barW + 2} height="3" rx="1.5" fill={today ? '#ffe08a' : '#ffb45f'} />}
            {today && <circle cx={p.x} cy={p.y - 8} r="3.5" fill="#fbbf24" stroke="#07090f" strokeWidth="2"/>}
        ...
        {points.map((p, i) => (i % 7 === 0 || i === points.length - 1) ? <text key={`d-${p.date}`} x={p.x} y={H-10} textAnchor="middle" fontSize="11" fill="rgba(230,234,245,.4)">{formatDate(p.date)}</text> : null)}
      </svg>}
    </div>
    ...
    <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[10px] text-ink-500"><span className="flex items-center gap-1.5"><i className="h-2 w-3 rounded-sm bg-ember-500"/>Daily score</span><span className="flex items-center gap-1.5"><i className="h-2 w-3 rounded-sm bg-gold-400"/>Today</span><span className="ml-auto text-ink-600">0–100%</span></div>
  </div>;
}
