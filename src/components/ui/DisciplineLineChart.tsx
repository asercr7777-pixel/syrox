...
export function DisciplineLineChart({ data, height = 310, showToday = true, animate = true }: DisciplineLineChartProps) {
  ...
  const W = 960, H = 420;
  const pad = { top: 46, right: 34, bottom: 96, left: 48 };
  ...
  return <section className="w-full" aria-label="Discipline performance chart">
    ...
    <div className="mb-3 flex items-center gap-2 overflow-x-auto pb-1">
      <Metric icon="◈" label="7D AVG" value={`${metrics.avg}%`} /><Metric icon="✦" label="PEAK" value={`${metrics.best}%`} />
      ...
      {points.length === 0 ? <div className="relative flex items-center justify-center py-20 text-sm text-ink-500">No discipline history yet.</div> : <div className="relative">
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={height} preserveAspectRatio="none" className="block w-full select-none">
          ...
          <g clipPath={`url(#${clipId})`}>
            ...
            {points.length > 1 && <path d={smoothLine} fill="none" stroke="rgba(255,255,255,.55)" strokeWidth="1" strokeLinecap="round" opacity=".45" className={animate ? 'chart-shine-sweep' : ''}/>} 
            {points.map((p, i) => <g key={p.date} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}><rect x={Math.max(pad.left, p.x-14)} y={pad.top} width="28" height={innerH} fill="transparent" className="cursor-crosshair"/>{hovered === i && i !== metrics.idx && <><circle cx={p.x} cy={p.y} r="12" fill="#a78bfa" fillOpacity=".07"/><circle cx={p.x} cy={p.y} r="4.5" fill="#ddd6fe" stroke="#080a10" strokeWidth="2.5"/></>}</g>)}
          </g>
          {today && showToday && <g><line x1={today.x} x2={today.x} y1={pad.top} y2={pad.top+innerH} stroke="#a78bfa" strokeOpacity=".16" strokeDasharray="4 8"/><circle cx={today.x} cy={today.y} r="20" fill="#8b5cf6" fillOpacity=".055" className="chart-today-pulse"/><circle cx={today.x} cy={today.y} r="9" fill="#c4b5fd" fillOpacity=".16" className="chart-today-pulse"/><circle cx={today.x} cy={today.y} r="5.5" fill="#f5f3ff" stroke="#171326" strokeWidth="3"/><g transform={`translate(${Math.max(8, Math.min(W-116, today.x-53))},${Math.max(8, today.y-57)})`}><rect width="106" height="29" rx="10" fill="#121522" stroke="rgba(196,181,253,.34)"/><text x="53" y="18" textAnchor="middle" fontSize="9" fontWeight="800" fill="#ddd6fe">TODAY · {today.score}%</text></g></g>}
          {points.map(p => <g key={`axis-${p.date}`}><line x1={p.x} x2={p.x} y1={pad.top+innerH} y2={pad.top+innerH+7} stroke="rgba(255,255,255,.16)" strokeWidth="1"/><text x={p.x} y={H-42} textAnchor="middle" fontSize="6.5" fontWeight="700" letterSpacing=".35" fill="rgba(255,255,255,.28)">{formatDay(p.date)}</text><text x={p.x} y={H-22} textAnchor="middle" fontSize="8.5" fontWeight="700" fill="rgba(255,255,255,.46)">{formatDate(p.date)}</text></g>)}
        </svg>
