...
export function Auth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      else toast({ title: 'Welcome to Forged!', message: 'Account created. You are now signed in.', type: 'success' });
    }
    setLoading(false);
  };
    ...
    <div className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden p-3 sm:p-4">
      ...
      <div className="relative w-full max-w-md py-3 sm:py-6">
        <div className="mb-6 text-center sm:mb-8">
          <div className="mb-3 inline-flex h-14 w-14 items-center justify-center rounded-2xl border border-ember-500/30 bg-gradient-to-br from-ember-500/20 to-shadow-500/20 sm:mb-4 sm:h-16 sm:w-16">
            <Shield className="text-ember-400" size={30} />
          </div>
          <h1 className="font-display text-3xl font-bold text-gradient-ember">FORGED</h1>
          <p className="mt-1 text-xs text-ink-300 sm:text-sm">Arise, Hunter. Your awakening begins now.</p>
        </div>

        <div className="card p-4 sm:p-6 md:p-8">
          <div className="mb-5 flex gap-1 rounded-xl border border-white/5 bg-ink-950/60 p-1 sm:mb-6">
            <button type="button" onClick={() => setMode('login')} className={`flex-1 rounded-lg py-2.5 text-sm font-semibold transition ${mode === 'login' ? 'bg-ember-500/20 text-ember-400' : 'text-ink-300 hover:text-ink-100'}`}>Sign In</button>
            <button type="button" onClick={() => setMode('signup')} className={`flex-1 rounded-lg py-2.5 text-sm font-semibold transition ${mode === 'signup' ? 'bg-ember-500/20 text-ember-400' : 'text-ink-300 hover:text-ink-100'}`}>Sign Up</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={!canSubmit} className="btn-primary w-full py-3 disabled:opacity-50">{loading ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}{mode === 'login' ? 'Enter Forged' : 'Begin Awakening'}</button>
          </form>

          <p className="mt-5 text-center text-xs text-ink-400">{mode === 'login' ? 'New to Forged? ' : 'Already awakened? '}<button type="button" onClick={() => setMode(mode === 'login' ? 'signup' : 'login')} className="font-semibold text-ember-400 hover:text-ember-300">{mode === 'login' ? 'Create an account' : 'Sign in instead'}</button></p>
        </div>
