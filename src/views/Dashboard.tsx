    ...
    <div className="stryven-command-deck space-y-6 pb-8">
      <section className="stryven-hero-panel relative overflow-hidden rounded-[28px] border border-white/10 bg-[#080808] p-6 sm:p-8 lg:p-10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.35fr_.65fr] lg:items-end">
          <div>
            <div className="mb-5 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.32em] text-ember-400"><span className="h-px w-8 bg-ember-500" />Command Center</div>
      ...
      <section className="stryven-stat-strip grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-4" aria-label="Hunter status">
      ...
      <section className="stryven-mission-board rounded-2xl border border-white/10 bg-[#090909]" aria-labelledby="today-missions"><div className="flex flex-col gap-3 border-b border-white/5 p-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.25em] text-ember-400">Live objectives</p><h2 id="today-missions" className="mt-1 font-display text-xl font-black uppercase">Today’s missions</h2></div><span className="stryven-status is-online">{totalDone} completed / {totalTasks} total</span></div><div className="grid gap-2 p-3 sm:grid-cols-2 lg:grid-cols-3">{allTasks.map((task, i) => <MissionRow key={task.id} index={i + 1} kind={task.kind} emoji={task.emoji} label={task.label} xp={task.points} done={task.kind === 'core' ? !!state.coreCompleted[task.id] : !!state.customCompleted[task.id]} onClick={() => task.kind === 'core' ? toggleCoreTask(task.id) : toggleCustomTask(task.id)} />)}{allTasks.length === 0 && <div className="col-span-full py-10 text-center text-sm text-ink-500">No missions configured.</div>}</div></section>
