...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  return (
    <div className="stryven-command-deck space-y-6 pb-8">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.35fr_.65fr] lg:items-end">
          <div>
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="min-w-0"><p className="text-xs uppercase tracking-[.2em] text-ink-500">Hunter identity</p><h1 className="mt-1 truncate font-display text-3xl font-black sm:text-5xl" style={{ color: state.nameColor }}>{state.username}</h1><p className="mt-2 text-sm text-ink-300">{rank.name}<span className="mx-2 text-ink-700">/</span>Level {state.level}<span className="mx-2 text-ink-700">/</span>{state.streak} day streak</p></div>
            </div>
          </div>
      ...
      <section className="stryven-stat-strip grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-4">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        <div className="stryven-command-card rounded-2xl border border-white/10 bg-[#0a0a0a] p-5 sm:p-6"><div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.25em] text-ember-400">Daily operation</p><h2 className="mt-1 font-display text-xl font-black uppercase">Mission progress</h2></div><span className="font-display text-3xl font-black tabular-nums">{Math.round(dailyPct)}%</span></div><div className="h-2 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-gradient-to-r from-ember-500 to-gold-500 transition-all duration-700" style={{ width: `${dailyPct}%` }} /></div><div className="mt-4 flex justify-between text-xs text-ink-500"><span>{state.dailyXp.toLocaleString()} / {state.dailyCap.toLocaleString()} XP</span><span>{completedCount} core missions cleared</span></div><button onClick={() => onNavigate('tasks')} className="mt-6 flex w-full items-center justify-between border-t border-white/5 pt-4 text-left text-sm font-bold text-ink-200 hover:text-ember-400">Open mission board <ArrowUpRight size={16} /></button></div>
      ...
      <section className="stryven-mission-board rounded-2xl border border-white/10 bg-[#090909]"><div className="flex flex-col gap-3 border-b border-white/5 p-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.25em] text-ember-400">Live objectives</p><h2 className="mt-1 font-display text-xl font-black uppercase">Today’s missions</h2></div><span className="text-xs text-ink-500">{totalDone} completed / {totalTasks} total</span></div><div className="grid gap-2 p-3 sm:grid-cols-2 lg:grid-cols-3">{allTasks.map((task, i) => <MissionRow key={task.id} index={i + 1} kind={task.kind} emoji={task.emoji} label={task.label} xp={task.points} done={task.kind === 'core' ? !!state.coreCompleted[task.id] : !!state.customCompleted[task.id]} onClick={() => task.kind === 'core' ? toggleCoreTask(task.id) : toggleCustomTask(task.id)} />)}{allTasks.length === 0 && <div className="col-span-full py-10 text-center text-sm text-ink-500">No missions configured.</div>}</div></section>

      <section className="stryven-command-actions grid gap-2 sm:grid-cols-3"><CommandAction icon={<Dumbbell size={18} />} title="Training" text="Enter workout console" onClick={() => onNavigate('workout')} /><CommandAction icon={<Swords size={18} />} title="Dungeons" text="Select a challenge" onClick={() => onNavigate('dungeons')} /><CommandAction icon={<Trophy size={18} />} title="Rankings" text="View hunter ladder" onClick={() => onNavigate('leaderboard')} /></section>
    </div>
  );
}
