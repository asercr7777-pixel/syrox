...
export function Marketplace() {
  ...
  return (
    <div className="marketplace-shell space-y-6">
      ...
      {previewItem && <Modal open={!!previewItem} onClose={() => setPreviewItem(null)} title={previewItem.name}><div className="flex flex-col items-center gap-4 p-4">{renderArtwork(previewItem, 180)}<p className="text-center text-sm text-ink-300">{CATEGORY_LABELS[previewItem.category]}</p></div></Modal>}
    </div>
  );
}
