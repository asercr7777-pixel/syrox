...
import { HunterSkillWheel } from '../components/HunterSkillWheel';
  ...
  return <div className="space-y-6">
    <div className="card-premium relative overflow-hidden p-4 sm:p-6 md:p-8 page-enter">
      ...
      <div className="relative flex flex-col md:flex-row md:items-end gap-4">
        <div className="relative shrink-0"><div className="absolute -inset-3 rounded-full pointer-events-none" style={{ background: `radial-gradient(circle, ${aura?.color ?? rank.glow}35, transparent 70%)` }} /><div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-2xl flex items-center justify-center text-5xl bg-ink-900 border-4 overflow-hidden" style={{ borderColor: frame?.color ?? rank.color, boxShadow: `0 0 40px ${aura?.color ?? rank.glow}` }}>{avatarIsImage ? <img src={state.avatar} alt="Profile" className="h-full w-full object-cover" /> : state.avatar}</div><button onClick={() => inputRef.current?.click()} disabled={uploading} className="absolute -bottom-2 -right-2 flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-ink-900 text-ink-200 shadow-lg hover:text-white" title="Upload profile image"><Upload size={16} className={uploading ? 'animate-spin' : ''} /></button><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={handleAvatarUpload} className="hidden" /></div>
