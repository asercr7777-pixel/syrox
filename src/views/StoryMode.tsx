...
// Story completion is its own progression. External main-task progress is shown as a helpful signal,
// but it is never required to finish a story mission. If the same real task was already completed,
// the story still recognizes that progress immediately.
function missionDone(state: ReturnType<typeof useStore>['state'], mission: StoryMission) {
  return Boolean(state.storyCompletedMissions[mission.id]) || missionProgress(state, mission) >= mission.target;
}
...
export default function StoryMode() {
  ...
  const openMission = (mission: StoryMission) => {
    ...
    const alreadyDone = Boolean(state.storyCompletedMissions[mission.id]);
    setCutsceneLines(alreadyDone ? (mission.cutsceneAfter.length ? mission.cutsceneAfter : mission.cutsceneBefore) : (mission.cutsceneBefore.length ? mission.cutsceneBefore : [{ speaker: 'Shadow', voice: 'guardian', text: 'The path is waiting. Complete the story mission here, then return.', emotion: 'mysterious' }]));
    setCutsceneKind(alreadyDone ? 'missionAfter' : 'missionBefore'); setView('cutscene'); playSfx('door');
  };

  const finishCutscene = () => {
    if (!pendingMission) { setView('missions'); return; }
    // Story missions are completed from inside the story. Main Tasks can provide progress,
    // but they are not a gate anymore. This also fixes missions such as Wisdom of the Ancients.
  ...
  const selectedProgress = useMemo(() => selected.missions.map((m) => ({ mission: m, progress: missionProgress(state, m), done: missionDone(state, m) })), [selected, state]);
  const allMissionsDone = selected.missions.length > 0 && selected.missions.every((m) => Boolean(state.storyCompletedMissions[m.id]));
