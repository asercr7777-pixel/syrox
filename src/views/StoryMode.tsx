...
import { ArrowLeft, ArrowRight, BookOpen, Check, Coins, Map as MapIcon, Music, Play, Skull, Sparkles, Swords, Target, Trophy, VolumeX, Zap } from 'lucide-react';
...
function getChapter(number: number) { return CHAPTERS.find((chapter) => chapter.number === number) ?? CHAPTERS[0]; }
...
export default function StoryMode() {
  ...
  const openMission = (mission: StoryMission) => {
    setPendingMission(mission); setCutsceneLines(mission.cutsceneBefore.length ? mission.cutsceneBefore : [{ speaker: 'Shadow', voice: 'guardian', text: 'The path is waiting. Complete the task, then return.', emotion: 'mysterious' }]); setCutsceneKind('missionBefore'); setView('cutscene'); playSfx('door');
  };

  const finishCutscene = () => {
    if (!pendingMission) { setView('missions'); return; }
    if (cutsceneKind === 'missionBefore') {
      if (!missionDone(state, pendingMission)) { toast({ title: 'Mission not complete', message: 'Complete the real task, then return to the story.', type: 'info', icon: '📋' }); setView('missions'); return; }
  ...
  const startBoss = () => {
    ...
    setBossHp(selected.boss.hp); setBossPhase('intro'); setView('boss'); playSfx('boss_roar');
  };
  const attackBoss = () => {
    if (bossPhase !== 'battle') return;
    const next = Math.max(0, bossHp - (20 + Math.floor(Math.random() * 21))); setBossHp(next); playSfx('sword_clash');
    if (next === 0) { setBossPhase('defeat'); playSfx('success'); const boss = selected.boss; defeatStoryBoss(boss.id); if (boss.rewardTitle) unlockStoryAchievement(boss.rewardTitle); if (boss.rewardLore) unlockLore(boss.rewardLore); completeStoryMission(`boss_${boss.id}`, { xp: boss.xpReward, coins: boss.coinReward }); setReward({ xp: boss.xpReward, coins: boss.coinReward, title: boss.rewardTitle, lore: boss.rewardLore }); triggerConfetti(60); setTimeout(() => setView('reward'), 800); }
  };
  const claimReward = () => { setReward(null); if (selected.number < 30) { advanceStoryChapter(); setSelected(getChapter(selected.number + 1)); setView('map'); } else { setView('map'); toast({ title: 'Arc I complete', message: 'All 30 chapters are forged.', type: 'reward', icon: '🏆' }); } };
  ...
  if (view === 'boss') return <section className="mx-auto max-w-3xl space-y-5"><button onClick={() => setView('missions')} className="btn-ghost"><ArrowLeft size={16} /> Back to missions</button><div className="relative overflow-hidden rounded-3xl border border-rose-500/30 bg-black/60 p-6 text-center sm:p-10"><div className="text-6xl">{selected.boss.emoji}</div><p className="mt-3 text-[10px] uppercase tracking-[0.3em] text-rose-300">{selected.boss.title}</p><h1 className="font-display text-3xl font-black text-white">{selected.boss.name}</h1><p className="mx-auto mt-2 max-w-xl text-sm text-ink-300">{selected.boss.description}</p>{bossPhase === 'intro' ? <button onClick={() => setBossPhase('battle')} className="btn-primary mt-7"><Swords size={16} /> Begin Battle</button> : bossPhase === 'battle' ? <><div className="mx-auto mt-7 max-w-xl"><div className="mb-2 flex justify-between text-xs text-ink-400"><span>Boss HP</span><span>{bossHp} / {selected.boss.hp}</span></div><div className="h-4 overflow-hidden rounded-full bg-white/5"><div className="h-full bg-gradient-to-r from-rose-600 to-red-400 transition-all" style={{ width: `${bossHp / selected.boss.hp * 100}%` }} /></div></div><button onClick={attackBoss} className="btn-primary mt-7"><Swords size={16} /> Strike</button></> : <div className="mt-7 rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-5"><Trophy size={28} className="mx-auto text-emerald-300" /><h2 className="mt-2 font-display text-2xl font-black text-white">Victory</h2><p className="text-sm text-ink-300">Shadow watches as the gate falls.</p></div>}</div></section>;
