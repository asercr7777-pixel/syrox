...
function progress(state: ReturnType<typeof useStore>['state'], m: StoryMission) {
  switch (m.type) {
    ...
    case 'workout': { const today = dateKey(Date.now()); const sessions = state.workoutSessions.filter(s => dateKey(s.completedAt) === today).length; const history = state.history.some(d => d.date === today && d.workoutCompleted); return Math.min(m.target, Math.max(state.workoutsCompletedToday, sessions, history ? 1 : 0)); }
...
export default function StoryModeSimpleV2() {
  ...
  return <WorldMap current={current} completed={state.storyChapter} open={open} />;
}

function WorldMap({ current, completed, open }: { current: number; completed: number; open: (c: StoryChapter) => void }) {
  const activeCount = Math.max(1, Math.min(30, completed + 1));
  const activeRoute = nodes.slice(0, activeCount).map(([x, y]) => `${x} ${y}`).join(' L ');
  const regionLabels = [
  ...
  return (
    <div className="relative min-h-[86vh] overflow-hidden rounded-[2rem] border border-white/10 bg-[#050607] text-white shadow-2xl">
      <style>{`.syrox-map-scroll::-webkit-scrollbar{height:12px}.syrox-map-scroll::-webkit-scrollbar-track{background:rgba(255,255,255,.06);border-radius:999px}.syrox-map-scroll::-webkit-scrollbar-thumb{background:rgba(255,255,255,.28);border-radius:999px}.syrox-map-scroll{scrollbar-width:auto;scrollbar-color:rgba(255,255,255,.28) rgba(255,255,255,.06)}`}</style>
      <header className="relative z-30 flex flex-wrap items-start justify-between gap-4 border-b border-white/10 bg-black/55 p-5 backdrop-blur-md sm:p-7">
        <div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.45em] text-amber-200/70"><Map size={14} /> SYROX WORLD MAP</div><h1 className="mt-2 text-4xl font-black tracking-tight sm:text-6xl">The Shattered Realm</h1><p className="mt-2 max-w-xl text-sm text-white/50">Chapter 01 → 30. Follow the connected road across the realm. The next territory unlocks as you progress.</p></div>
      ...
      <div className="relative z-20 px-3 pt-3 sm:px-7 sm:pt-5">
        ...
        <div className="syrox-map-scroll overflow-x-auto overflow-y-hidden rounded-[1.75rem] border border-amber-300/20 bg-black/40 shadow-inner">
          <div className="relative" style={{ width: MAP_W, height: MAP_H }}>
            <img src="/world-map.webp.jpg" alt="Syrox World Map" draggable={false} className="absolute inset-0 h-full w-full select-none object-cover" />
            <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-black/35" />
            <svg viewBox={`0 0 ${MAP_W} ${MAP_H}`} className="absolute inset-0 h-full w-full pointer-events-none" aria-hidden="true">
              <defs><filter id="syrox-route-glow"><feGaussianBlur stdDeviation="5" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter><linearGradient id="syrox-route-gradient" x1="0" y1="1" x2="1" y2="0"><stop offset="0%" stopColor="#e5a83b" /><stop offset="55%" stopColor="#ffe6a0" /><stop offset="100%" stopColor="#fff" /></linearGradient></defs>
              <path d={routeD} fill="none" stroke="rgba(0,0,0,.72)" strokeWidth="34" strokeLinecap="round" strokeLinejoin="round" />
              <path d={routeD} fill="none" stroke="rgba(255,231,157,.34)" strokeWidth="13" strokeLinecap="round" strokeLinejoin="round" filter="url(#syrox-route-glow)" />
              <path d={routeD} fill="none" stroke="url(#syrox-route-gradient)" strokeWidth="7" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="18 7" />
              <path d={activeCount > 1 ? `M ${activeRoute}` : `M ${nodes[0][0]} ${nodes[0][1]}`} fill="none" stroke="#fff" strokeWidth="9" strokeLinecap="round" strokeLinejoin="round" filter="url(#syrox-route-glow)" />
              {regionLabels.map(([name, x, y]) => <text key={name} x={x} y={y} fill="rgba(255,255,255,.72)" fontSize="20" fontWeight="900" letterSpacing="5" textAnchor="middle" style={{ textShadow: '0 3px 10px rgba(0,0,0,.9)' }}>{name}</text>)}
            </svg>
            ...
            {nodes.map(([x, y], index) => {
              ...
              return <button key={number} onClick={() => unlocked && open(chapters[index])} disabled={!unlocked} aria-label={`Chapter ${number}${!unlocked ? ', locked' : ''}`} className="absolute z-10 -translate-x-1/2 -translate-y-1/2 disabled:cursor-not-allowed" style={{ left: x, top: y }}>
                <span className={`relative flex h-[68px] w-[68px] items-center justify-center rounded-full border-2 shadow-[0_10px_30px_rgba(0,0,0,.55)] transition-transform ${currentNode ? 'scale-125 border-white bg-black/90' : conquered ? 'border-amber-200 bg-black/85' : unlocked ? 'border-white/80 bg-black/75 hover:scale-110' : 'border-white/20 bg-black/75 opacity-75'}`}>
                  {currentNode && <span className="absolute inset-[-9px] animate-pulse rounded-full border border-white/70" />}
                  {conquered ? <Check size={22} /> : unlocked ? <span className="text-lg font-black">{String(number).padStart(2, '0')}</span> : <Lock size={18} />}
                  <span className={`absolute -bottom-7 whitespace-nowrap rounded-md px-2 py-1 text-[9px] font-black tracking-[.16em] shadow-lg ${currentNode ? 'bg-white text-black' : 'bg-black/80 text-white/75'}`}>CH {String(number).padStart(2, '0')}</span>
                </span>
              </button>;
            })}
          </div>
      ...
      <footer className="relative z-20 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 bg-black/45 px-5 py-4 text-[9px] font-black uppercase tracking-[.25em] text-white/35 sm:px-7"><span><span className="mr-2 inline-block h-2 w-2 rounded-full bg-amber-200" /> {completed}/30 chapters conquered</span><span>Current Chapter {current}/30</span><span className="flex items-center gap-2"><Skull size={12} /> Complete the route to reach each Boss Citadel</span></footer>
    </div>
  );
}
