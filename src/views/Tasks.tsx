...
export function Tasks() {
  ...
  const openCustomEdit = (id: string) => { const task = state.customTasks.find((t) => t.id === id); if (!task) return; setForm({ label: task.label, emoji: task.emoji, points: Math.min(MAX_TASK_XP, task.points), description: '', category: 'body' }); setCustomEditId(id); };
  const saveMain = () => {
    const label = form.label.trim();
    if (!label) return toast({ title: 'Mission name is required', type: 'error' });
    const points = Math.max(1, Math.min(MAX_TASK_XP, Math.floor(Number(form.points) || 0)));
  ...
  const completedXP = active.filter((t) => !!state.coreCompleted[t.id]).reduce((sum, t) => sum + Math.min(MAX_TASK_XP, t.points), 0);
  const missionPct = active.length ? Math.round((completed / active.length) * 100) : 0;
  const dailyXP = Math.max(0, state.dailyXp);
    ...
    <header className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/30 p-5 sm:p-7">
      ...
      <div className="relative">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 sm:min-w-[330px]"><Metric label="Missions" value={`${completed}/${active.length}`} /><Metric label="Mission XP" value={`${completedXP}`} /><Metric label="XP Today" value={`${dailyXP}`} /></div>
        </div>
        <div className="mt-6"><ProgressPanel label="Mission progress" value={`${completed}/${active.length}`} percent={missionPct}/></div>
      </div>
    </header>

    <section className="card overflow-hidden">
      <SectionHeader icon={<ListChecks size={18}/>} title="Core Missions" subtitle="Primary daily progression objectives · max 200 XP each" action="Add Mission" onAction={openMainAdd}/>
      <div className="divide-y divide-white/5">
        {sorted.map((task, idx) => {
          const done = !!state.coreCompleted[task.id]; const busy = busyIds.has(task.id); const cat = CATEGORIES.find((c) => c.id === task.category); const taskXp = Math.min(MAX_TASK_XP, task.points);
...
function ProgressPanel({label,value,percent}:{label:string;value:string;percent:number}) { return <div className="rounded-xl border border-white/5 bg-white/[0.025] p-4"><div className="flex items-center justify-between gap-3 text-[10px] font-bold uppercase tracking-widest"><span className="text-ink-400">{label}</span><span className="text-white">{value}</span></div><div className="stryven-progress mt-3"><div className="stryven-progress-fill" style={{width:`${percent}%`}}/></div></div>; }
...
function TaskForm({form,setForm,showDetails,onSave,onCancel,saveLabel}:{form:FormData;setForm:Dispatch<SetStateAction<FormData>>;showDetails?:boolean;onSave:()=>void;onCancel:()=>void;saveLabel:string}) {
  return <div className="space-y-4 p-1"><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-ink-400">Name</span><input autoFocus value={form.label} onChange={(e)=>setForm({...form,label:e.target.value})} maxLength={80} className="stryven-input w-full" placeholder="e.g. Train for 60 minutes"/></label>
    <div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-ink-400">XP Reward</span><input type="number" min={1} max={MAX_TASK_XP} value={form.points} onChange={(e)=>setForm({...form,points:Math.min(MAX_TASK_XP,Math.max(1,Number(e.target.value)||1))})} className="stryven-input w-full"/><span className="mt-1 block text-[10px] text-ink-500">Maximum {MAX_TASK_XP} XP per task</span></label><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-ink-400">Category</span><select value={form.category} onChange={(e)=>setForm({...form,category:e.target.value as MainTask['category']})} disabled={!showDetails} className="stryven-input w-full">{CATEGORIES.map((c)=><option key={c.id} value={c.id}>{c.label}</option>)}</select></label></div>
