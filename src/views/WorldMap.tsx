...
import { ArrowRight, BookOpen, Check, Map, Swords, Zap } from 'lucide-react';
      ...
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-3xl border border-ember-500/25 bg-[#08090d]/95 p-5 sm:p-7">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-300 sm:text-base">Arc I follows your awakening inside a world damaged by millions of small surrenders. Thirty chapters trace one continuous path—from the first signal to the World Core. Every chapter is shown in story order; the map is not locked by level. Shadow will guide you from inside the story itself.</p>
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="chip border border-ember-500/25 bg-ember-500/10 text-ember-300"><BookOpen size={12} /> 30 Chapters</span>
              <span className="chip border border-violet-500/25 bg-violet-500/10 text-violet-300">Shadow · Story Guide</span>
              <span className="chip border border-white/10 bg-white/[0.03] text-ink-300"><Swords size={12} /> Story Progression</span>
            </div>
          </div>
          <div className="hidden lg:flex h-28 w-28 shrink-0 items-center justify-center rounded-full border border-ember-400/40 bg-[radial-gradient(circle,rgba(245,158,11,.3),rgba(10,10,14,.95)_60%)] shadow-[0_0_45px_rgba(245,158,11,.12)]"><Map size={34} className="text-ember-300" /></div>
        </div>
      </motion.div>
